      ...
      <table className="w-full text-left">
        <thead>
          <tr className="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-xl">
            <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">Analysis Payload</th>
            <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 text-center">Severity Node</th>
            <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 text-right">Coefficient</th>
            <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 text-right">Timestamp</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-zinc-900/40">
          {data.map((mr, index) => (
            <motion.tr
              ...
              whileHover={{ backgroundColor: "rgba(255, 255, 255, 0.02)", x: 4, transition: { duration: 0.15 } }}
              onClick={() => router.push(`/mr/${mr.id}`)}
              className="group cursor-pointer relative"
            >
              {/* Left Indicator Bar */}
              <td className="absolute left-0 top-0 bottom-0 w-1 p-0 overflow-hidden">
                <div className={`h-full w-full ${getScoreColor(mr.risk_score)} opacity-40 group-hover:opacity-100 transition-opacity`} />
              </td>

              <td className="px-10 py-8">
                <div className="flex items-center gap-6">
                  <div className={`w-2 h-2 rounded-full shrink-0 ${getScoreColor(mr.risk_score)} shadow-[0_0_12px_rgba(255,255,255,0.1)] group-hover:shadow-[0_0_15px_currentColor] transition-all`} />
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-3 text-[11px] font-bold text-zinc-600 group-hover:text-zinc-500 transition-colors uppercase tracking-[0.05em]">
                      ...
                      <span className="w-1 h-1 rounded-full bg-zinc-800" />
                      <span className="tabular-nums font-black text-zinc-700">!{mr.mr_iid}</span>
                    </div>
                  </div>
                </div>
              </td>
              ...
              <td className="px-10 py-8 text-right">
                <div className="flex flex-col items-end gap-1">
                  ...
                  <span className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.2em]">FACTOR</span>
                </div>
              </td>
              <td className="px-10 py-8 text-right">
